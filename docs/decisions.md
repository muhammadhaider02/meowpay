...
| **Live balance updates over Supabase Realtime** | A subscription needs the table published through PostgREST and row level security policies to authorise it. These tables sit in a private schema with RLS on and no policies, so buying liveness would mean giving a browser a route to the ledger, which is the one thing that layout exists to prevent. `useSession` refetches when the window regains focus instead, which is the only moment a balance has to be current |
...
|---|---|
| **The front end tests the request client and the key store, not the components** | 43 tests cover `lib/api.ts` and `lib/idempotency.ts`, which between them are the only parts of the browser that can cause a double spend. The forms and the rendering carry none, so a broken layout is caught by opening the page rather than by a suite. That is the cheap failure to find; the expensive one is covered. |
...
| **A signed-out token stays valid until it expires** | Nothing is introspected per request, so signing out revokes the refresh token and not the outstanding access token. Mitigated by the 900 second token lifetime [deployment.md](deployment.md) requires be set, rather than the 3600 default a fresh project ships with. `session_id` is in the claims, so a denylist is the available seam if it ever needs to be tighter. This is the one security cost of verifying tokens rather than introspecting them, and it buys not having the auth server in the path of every transfer |
